...
module Optional
  class Memory
    def initialize()
      @command_name = "memory"
      @command_proc = Proc.new() { |user, args| on_memory(user, args) }
    end
    ...
    def iter_hash(user, hash)
      hash.each do |key, value|
        ...
        if value.is_a?(Hash)
          Network.send(user, Numeric.RPL_DEBUGMSG(user.nick, sprintf("%s:", key)))
          iter_hash(user, value)
        else
          Network.send(user, Numeric.RPL_DEBUGMSG(user.nick, sprintf("%s = %s", key, value)))
        end
      end
    end
